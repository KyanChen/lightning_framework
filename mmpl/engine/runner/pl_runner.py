...
class PLRunner:
    def __init__(
        ...
        predict_dataloader: Optional[Union[DataLoader, Dict]] = None,
        ...
        # if mmengine.exists(cfg.trainer_cfg['default_root_dir']):
        mmengine.mkdir_or_exist(cfg.trainer_cfg['default_root_dir'])
        ...
        compiled_model = self.trainer_cfg.pop('compiled_model', False)

        loggers = self.trainer_cfg.get('logger', None)
        if loggers is not None:
            ...
            for logger in loggers:
                if logger.get('save_dir', None) is None:
                    logger['save_dir'] = cfg.trainer_cfg['default_root_dir']
                tmp_loggers.append(LOGGERS.build(logger))
            loggers = tmp_loggers
        self.trainer_cfg['logger'] = loggers

        self.trainer = pl.Trainer(**self.trainer_cfg)
        self.pler = self.build_pler(model_cfg)
        if compiled_model:
            # default, reduce-overhead, and max-autotune.
            self.pler = torch.compile(self.pler)
        self._datamodule_cfg = datamodule_cfg
    ...
    def visualize(self,
        ...
        if self.visualizer is not None:
            self.visualizer.visualize(results, show=show, **kwargs)
    ...
    def from_cfg(cls, cfg: ConfigType) -> 'Runner':
        ...
        runner = cls(
            ...
            # predict_dataloader=cfg.get('predict_dataloader'),
    ...
    def build_pler(self, pler: Union[pl.LightningModule, Dict]) -> pl.LightningModule:
        if isinstance(pler, pl.LightningModule):
            return pler
        elif isinstance(pler, dict):
            pler = build_pler(pler)
            return pler  # type: ignore
        else:
    ...
    def run(self, status, *args, **kwargs):
        ...
        self.datamodule = self.build_datamodule(self._datamodule_cfg)
        return trainer_func(model=self.pler, datamodule=self.datamodule, *args, **kwargs)
