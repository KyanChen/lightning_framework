...
def model_forward(results, model, tokenizer, text_features, size, mean, std, device='cuda:0'):
    ...
    return {'img_dense_embs': img_dense_embs, 'logits_per_image': logits_per_image}
...
def main():
    ...
    for dataset in cache_datasets:
        ...
        for i, item in zip(range(len(dataset)), dataset):
            ...
            mmengine.dump(cache_data, f"{args.output_dir}/{osp.splitext(osp.basename(img_path))[0]}.pkl")
